 ...
 * @author Deathscythvi
 */
public class ServiceRideDriver implements Interfaces.InterfaceRideDriver {

    Conn cnx;

    public ServiceRideDriver() {

        this.cnx = Conn.getInstance();
    }

    @Override
    public void ajouterRideDriver(RideDriver dr) {
        try {
            String req = "INSERT INTO ridedriver (price,citySource,placeSource,longSource,latSource,cityDestination,placeDestination,longDestination,latDestination,dateTimeSource,nbrPlaces,confortVoiture,marqueVoiture,modeleVoiture,handicap,animal,haveAnimal,haveLuggage,luggageMass,music,musicTaste,smoking,allowSmoking,idDriver, frequency, frequencyUnit, idFiliale) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NULL,NULL,NULL)";
            PreparedStatement ps = cnx.getConnection().prepareStatement(req);

            ps.setDouble(1, dr.getPrice());
            ps.setString(2, dr.getCitySource());
            ps.setString(3, dr.getPlaceSource());
            ps.setDouble(4, dr.getLongSource());
            ps.setDouble(5, dr.getLatSource());
            ps.setString(6, dr.getCityDestination());
            ps.setString(7, dr.getPlaceDestination());
            ps.setDouble(8, dr.getLongDestination());
            ps.setDouble(9, dr.getLatDestination());
            ps.setDate(10, new java.sql.Date((dr.getDateTimeSource()).getTime()));
            ps.setInt(11, dr.getNbrPlaces());
            ps.setString(12, dr.getConfortVoiture());
            ps.setString(13, dr.getMarqueVoiture());
            ps.setString(14, dr.getModeleVoiture());
            ps.setByte(15, dr.getHandicap());
            ps.setByte(16, dr.getAnimal());
            ps.setByte(17, dr.getHaveanimal());
            ps.setByte(18, dr.getHaveluggage());
            ps.setFloat(19, dr.getLuggageMass());
            ps.setByte(20, dr.getMusic());
            ps.setString(21, dr.getMusicTaste());
            ps.setByte(22, dr.getSmoking());
            ps.setByte(23, dr.getAllowsmoking());
            ...
            Logger.getLogger(ServiceRideDriver.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    @Override
    public void ModifierRideDriver(RideDriver dr) {
        try {
            String req = "UPDATE ridedriver SET price=? ,citySource=? ,placeSource=? ,longSource=? ,latSource=? ,cityDestination=? ,placeDestination=? ,longDestination=? ,latDestination=? ,dateTimeSource=?, nbrPlaces=? ,confortVoiture=? ,marqueVoiture=?, modeleVoiture=?,handicap=? ,animal=?, haveAnimal=?, haveLuggage=?, luggageMass=? ,music=? ,musicTaste=?, smoking=? ,allowSmoking=?, idDriver=?,frequency=NULL,frequencyUnit=NULL,idFiliale=NULL WHERE id = ?";
            PreparedStatement ps = cnx.getConnection().prepareStatement(req);
            ps.setDouble(1, dr.getPrice());
            ps.setString(2, dr.getCitySource());
            ps.setString(3, dr.getPlaceSource());
    ...
    @Override
    public void DeleteRideDriver(RideDriver dr) {
        try {
    ...
    public void AffichertousRideDriver() {
        try {
            String req = "select * from ridedriver WHERE citySource LIKE '%f%'";
            PreparedStatement ps = cnx.getConnection().prepareStatement(req);
    ...
    public List<RideDriver> RechercherMesDrive() {
        String req = "SELECT * FROM `ridedriver` WHERE `idDriver`=? ";
        PreparedStatement ps;

        List<RideDriver> rec = new ArrayList<>();
        try {
    ...
    public List<RideDriver> RechercherMesDrivernotall() {
        ...
        try {
            ...
            while (rs.next()) {
                RideDriver r = new RideDriver(rs.getInt("id"), rs.getDouble("price"), rs.getString("citySource"), rs.getString("placeSource"), rs.getString("cityDestination"), rs.getString("placeDestination"), rs.getDate("dateTimeSource"));
                rec.add(r);
            }
    ...
    public List<RideDriver> RechercherMesDriverauto(String c) {
        String req = "SELECT id,price,citySource,placeSource,cityDestination,placeDestination,dateTimeSource FROM `ridedriver` WHERE `idDriver`=? AND citySource LIKE '%"+c+"%' OR `idDriver`=? AND placeSource LIKE '%"+c+"%' OR `idDriver`=? AND placeDestination LIKE '%"+c+"%' OR `idDriver`=? AND citySource LIKE '%"+c+"%'";
        PreparedStatement ps;
        ...
        try {
            ps = cnx.getConnection().prepareStatement(req);
            ps.setInt(1, Integer.parseInt(getCurrentId()));
             ps.setInt(2, Integer.parseInt(getCurrentId()));
             ps.setInt(3, Integer.parseInt(getCurrentId()));
             ps.setInt(4, Integer.parseInt(getCurrentId()));
